...
# ---- TOMCAT-SESSION-001 (中间件安全/Tomcat) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
ITEM_JTYPE='numeric_leq'
ITEM_TE='fail'
ITEM_CMD='grep -rhoE "connectionTimeout=\"[0-9]+\"" $CATALINA_HOME/conf/server.xml 2>/dev/null | grep -oE "[0-9]+" | tail -1'
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
...
ITEM_JVAL='300000'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"TOMCAT-SESSION-001","category":"中间件安全","subsystem":"Tomcat","name":"定时登出(connectionTimeout)","severity":"medium","expected":"≤ 300000","remediation":"在 server.xml 的 Connector 设置 connectionTimeout=300000（毫秒）","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-SSL-002 (中间件安全/Tomcat) ----
ITEM_CHECK='manual'
ITEM_MTYPE='manual'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_PORT=''
ITEM_JVAL=''
ITEM_MDESC='使用 HTTPS 协议可成功访问 Tomcat 管理页面则合规，否则不合规。需人工访问验证。'
ITEM_STATIC='"item_id":"TOMCAT-SSL-002","category":"中间件安全","subsystem":"Tomcat","name":"HTTP 加密协议(HTTPS 管理页面)","severity":"high","expected":"","remediation":"配置 keytool 证书并在 server.xml 增加 SSL Connector(SSLEnabled=true)，重启 Tomcat","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-ERR-003 (中间件安全/Tomcat) ----
ITEM_CHECK='manual'
ITEM_MTYPE='manual'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_PORT=''
ITEM_JVAL=''
ITEM_MDESC='访问不存在的页面(如 /manager12345)应跳转到自定义错误页。需人工验证。'
ITEM_STATIC='"item_id":"TOMCAT-ERR-003","category":"中间件安全","subsystem":"Tomcat","name":"错误页面重定向","severity":"low","expected":"","remediation":"在 web.xml 增加 <error-page> 指向自定义错误页并重启","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-ROLE-004 (中间件安全/Tomcat) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_TE='fail'
ITEM_CMD='grep -iE "admin|manager" $CATALINA_HOME/conf/tomcat-users.xml 2>/dev/null | head -2 | tr '\''\n'\'' '\'' '\'''
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
ITEM_PORT=''
ITEM_JVAL='(?i)admin|manager'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"TOMCAT-ROLE-004","category":"中间件安全","subsystem":"Tomcat","name":"配置用户最小权限(远程管理角色)","severity":"medium","expected":"匹配 (?i)admin|manager","remediation":"在 tomcat-users.xml 的 <tomcat-users> 下为管理用户授予 admin-gui/manager-gui 等最小所需角色","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-PORT-005 (中间件安全/Tomcat) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_TE='fail'
ITEM_CMD='grep -rhoE "port=\"[0-9]+\"" $CATALINA_HOME/conf/server.xml 2>/dev/null | grep -oE "[0-9]+" | head -1'
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
...
ITEM_JVAL='8080'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"TOMCAT-PORT-005","category":"中间件安全","subsystem":"Tomcat","name":"更改默认管理端口","severity":"low","expected":"不等于 8080","remediation":"修改 server.xml Connector port 为非 8080 并重启","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-LIST-006 (中间件安全/Tomcat) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
ITEM_JTYPE='equals'
ITEM_TE='fail'
ITEM_CMD='grep -A2 "listings" $CATALINA_HOME/conf/web.xml 2>/dev/null | grep -oE "false|true" | tail -1'
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
...
ITEM_JVAL='false'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"TOMCAT-LIST-006","category":"中间件安全","subsystem":"Tomcat","name":"禁止目录列出(listings)","severity":"medium","expected":"等于 false","remediation":"在 web.xml 的 listings 参数设为 false 并重启","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-LOG-007 (中间件安全/Tomcat) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_TE='fail'
ITEM_CMD='grep -c "AccessLogValve" $CATALINA_HOME/conf/server.xml 2>/dev/null || echo 0'
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
...
ITEM_JVAL='1'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"TOMCAT-LOG-007","category":"中间件安全","subsystem":"Tomcat","name":"记录用户登录行为(AccessLogValve)","severity":"medium","expected":"≥ 1","remediation":"在 server.xml 的 <Host> 中启用 AccessLogValve 记录访问日志","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-SHUT-008 (中间件安全/Tomcat) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_TE='fail'
ITEM_CMD='grep -rhoE "shutdown=\"[^\"]*\"" $CATALINA_HOME/conf/server.xml 2>/dev/null | head -1'
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
...
ITEM_JVAL='SHUTDOWN'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"TOMCAT-SHUT-008","category":"中间件安全","subsystem":"Tomcat","name":"防止恶意关闭 Tomcat 服务(shutdown 口令)","severity":"medium","expected":"不包含 SHUTDOWN","remediation":"将 server.xml 中 <Server port=\"8005\" shutdown=\"SHUTDOWN\"> 的 shutdown 改为复杂字符串并重启","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-USER-009 (中间件安全/Tomcat) ----
ITEM_CHECK='auto'
ITEM_MTYPE='process'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_MDESC=''
ITEM_STATIC='"item_id":"TOMCAT-USER-009","category":"中间件安全","subsystem":"Tomcat","name":"服务账号管理(不以 root 运行)","severity":"high","expected":"不包含 running:root","remediation":"创建 tomcat 用户/组并以 tomcat 身份启动服务","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-BANNER-010 (中间件安全/Tomcat) ----
ITEM_CHECK='manual'
ITEM_MTYPE='manual'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_PORT=''
ITEM_JVAL=''
ITEM_MDESC='执行 $CATALINA_HOME/bin/version.sh 查看 server.info/server.number/server.built 应为自定义内容。需人工核查。'
ITEM_STATIC='"item_id":"TOMCAT-BANNER-010","category":"中间件安全","subsystem":"Tomcat","name":"更改系统 Banner 信息","severity":"low","expected":"","remediation":"修改 lib/catalina.jar 中 Serverinfo.properties 的 server.info/server.number/server.built 以掩盖真实版本","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-PWD-011 (中间件安全/Tomcat) ----
ITEM_CHECK='manual'
ITEM_MTYPE='manual'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_PORT=''
ITEM_JVAL=''
ITEM_MDESC='检查 tomcat-users.xml 中所有账号口令应满足长度>=8 且含数字/大小写/特殊符号中至少两类。需人工核查。'
ITEM_STATIC='"item_id":"TOMCAT-PWD-011","category":"中间件安全","subsystem":"Tomcat","name":"口令复杂度(tomcat-users)","severity":"medium","expected":"","remediation":"为所有 Tomcat 管理账号设置满足复杂度要求的口令","reference":"等保2.0 8.1.4"'
_run_item

# ---- TOMCAT-CONN-012 (中间件安全/Tomcat) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
ITEM_JTYPE='numeric_leq'
ITEM_TE='fail'
ITEM_CMD='grep -rhoE "maxConnections=\"[0-9]+\"" $CATALINA_HOME/conf/server.xml 2>/dev/null | grep -oE "[0-9]+" | tail -1'
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
...
ITEM_JVAL='200'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"TOMCAT-CONN-012","category":"中间件安全","subsystem":"Tomcat","name":"限制最大连接数(maxConnections)","severity":"low","expected":"≤ 200","remediation":"在 server.xml Connector 设置 maxConnections=200 并重启","reference":"等保2.0 8.1.4"'
_run_item
# ---------------------------------------------------------------------------
# 主机元数据 + 汇总 + 输出
# ---------------------------------------------------------------------------
...
CATALOG_ID="mw_tomcat"
CATALOG_VER="1.0.0"
VERSIONS='{"db2":"1.0.0","db_mariadb":"1.0.0","db_mongodb":"1.0.0","db_mysql_linux":"1.0.0","db_mysql_win":"1.0.0","db_oracle_linux":"1.0.0","db_oracle_win":"1.0.0","db_postgresql":"1.0.0","db_redis":"1.0.0","db_sqlserver_win":"1.0.0","host_linux":"1.0.0","host_windows":"1.0.0","infra_docker":"1.0.0","infra_k8s":"1.0.0","infra_network":"1.0.0","mw_apache":"1.0.0","mw_iis":"1.0.0","mw_nginx":"1.0.0","mw_tomcat":"1.0.0"}'
