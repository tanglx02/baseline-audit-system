...
ITEM_JTYPE='equals'
ITEM_TE='fail'
ITEM_CMD='f="$HTTPD_HOME/conf/httpd.conf /etc/httpd/conf/httpd.conf /etc/apache2/apache2.conf"; u=$(grep -iE "^\s*User\s+" $f 2>/dev/null | awk '\''{print $2}'\'' | tail -1); g=$(grep -iE "^\s*Group\s+" $f 2>/dev/null | awk '\''{print $2}'\'' | tail -1); if [ -n "$u" ] && [ "$u" != "root" ] && [ -n "$g" ] && [ "$g" != "root" ]; then echo OK; else echo FAIL; fi'
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
...
ITEM_JVAL='OK'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"APACHE-USER-001","category":"中间件安全","subsystem":"Apache","name":"以指定用户和组运行服务(非 root)","severity":"high","expected":"等于 OK","remediation":"在 httpd.conf 设置 User apache 与 Group apache（专用账号）","reference":"等保2.0 8.1.4"'
_run_item

# ---- APACHE-PERM-002 (中间件安全/Apache) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
ITEM_JTYPE='equals'
ITEM_TE='fail'
ITEM_CMD='stat -c '\''%a'\'' $HTTPD_HOME/conf/httpd.conf /etc/httpd/conf/httpd.conf /etc/apache2/apache2.conf 2>/dev/null | head -1'
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
...
ITEM_JVAL='600'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"APACHE-PERM-002","category":"中间件安全","subsystem":"Apache","name":"httpd.conf 配置文件权限(600)","severity":"medium","expected":"等于 600","remediation":"chmod 600 httpd.conf","reference":"等保2.0 8.1.4"'
_run_item

# ---- APACHE-LOG-003 (中间件安全/Apache) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_TE='fail'
ITEM_CMD='grep -iE "ErrorLog|CustomLog|LogLevel" $HTTPD_HOME/conf/httpd.conf /etc/httpd/conf/httpd.conf /etc/apache2/apache2.conf 2>/dev/null | grep -vE '\''^#'\'' | head -3 | tr '\''\n'\'' '\'' '\'''
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
ITEM_PORT=''
ITEM_JVAL='(?=.*ErrorLog)(?=.*CustomLog)'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"APACHE-LOG-003","category":"中间件安全","subsystem":"Apache","name":"设置启用日志功能","severity":"medium","expected":"匹配 (?=.*ErrorLog)(?=.*CustomLog)","remediation":"在 httpd.conf 配置 ErrorLog、LogLevel notice、CustomLog 与 LogFormat combined","reference":"等保2.0 8.1.4"'
_run_item

# ---- APACHE-LOG-004 (中间件安全/Apache) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
ITEM_JTYPE='equals'
ITEM_TE='pass'
ITEM_CMD='bad=0; for f in $HTTPD_HOME/logs/*.log /var/log/httpd/*.log /var/log/apache2/*.log; do [ -f "$f" ] && p=$(stat -c '\''%a'\'' "$f" 2>/dev/null); [ -n "$p" ] && [ "$p" -gt 644 ] && bad=1; done; [ "$bad" -eq 0 ] && echo OK || echo FAIL'
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
...
ITEM_JVAL='OK'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"APACHE-LOG-004","category":"中间件安全","subsystem":"Apache","name":"日志文件访问权限(644)","severity":"low","expected":"等于 OK","remediation":"chmod -R 644 日志文件","reference":"等保2.0 8.1.4"'
_run_item

# ---- APACHE-ERR-005 (中间件安全/Apache) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
ITEM_JTYPE='regex_present'
ITEM_TE='fail'
ITEM_CMD='grep -iE "ErrorDocument" $HTTPD_HOME/conf/httpd.conf /etc/httpd/conf/httpd.conf /etc/apache2/apache2.conf 2>/dev/null | grep -vE '\''^#'\'' | head -2 | tr '\''\n'\'' '\'' '\'''
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
ITEM_PORT=''
ITEM_JVAL='ErrorDocument'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"APACHE-ERR-005","category":"中间件安全","subsystem":"Apache","name":"配置错误页面重定向","severity":"low","expected":"匹配 ErrorDocument","remediation":"在 httpd.conf 增加 ErrorDocument 400/401/403/404/500 指向自定义页面","reference":"等保2.0 8.1.4"'
_run_item

# ---- APACHE-LIST-006 (中间件安全/Apache) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_TE='pass'
ITEM_CMD='grep -iE "Options\s+.*Indexes" $HTTPD_HOME/conf/httpd.conf /etc/httpd/conf/httpd.conf /etc/apache2/apache2.conf 2>/dev/null | grep -vE '\''^#'\'' | head -2 | tr '\''\n'\'' '\'' '\'''
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
ITEM_PORT=''
ITEM_JVAL='(?i)Options\s+.*Indexes'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"APACHE-LIST-006","category":"中间件安全","subsystem":"Apache","name":"禁止显示目录结构(去掉 Indexes)","severity":"medium","expected":"不包含 (?i)Options\\s+.*Indexes","remediation":"在 Directory 的 Options 中去掉 Indexes 并重启 Apache","reference":"等保2.0 8.1.4"'
_run_item

# ---- APACHE-BANNER-007 (中间件安全/Apache) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
ITEM_JTYPE='regex_present'
ITEM_TE='fail'
ITEM_CMD='grep -iE "ServerSignature|ServerTokens" $HTTPD_HOME/conf/httpd.conf /etc/httpd/conf/httpd.conf /etc/apache2/apache2.conf 2>/dev/null | grep -vE '\''^#'\'' | head -2 | tr '\''\n'\'' '\'' '\'''
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
ITEM_PORT=''
ITEM_JVAL='(?=.*ServerSignature\s+Off)(?=.*ServerTokens\s+Prod)'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"APACHE-BANNER-007","category":"中间件安全","subsystem":"Apache","name":"隐藏版本号(ServerSignature/ServerTokens)","severity":"medium","expected":"匹配 (?=.*ServerSignature\\s+Off)(?=.*ServerTokens\\s+Prod)","remediation":"在 httpd.conf 设置 ServerSignature Off 与 ServerTokens Prod","reference":"等保2.0 8.1.4"'
_run_item

# ---- APACHE-FILE-008 (中间件安全/Apache) ----
ITEM_CHECK='manual'
ITEM_MTYPE='manual'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_PORT=''
ITEM_JVAL=''
ITEM_MDESC='检查 $HTTPD_HOME/htdocs、cgi-bin、manual 等缺省文件/目录是否存在，应删除。需人工核查。'
ITEM_STATIC='"item_id":"APACHE-FILE-008","category":"中间件安全","subsystem":"Apache","name":"删除缺省安装的文件","severity":"low","expected":"","remediation":"删除缺省 HTML/CGI/说明文件：rm -rf $APACHE_HOME/htdocs/* cgi-bin/* manual/*","reference":"等保2.0 8.1.4"'
_run_item

# ---- APACHE-SESSION-009 (中间件安全/Apache) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
...
ITEM_JVAL='OK'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"APACHE-SESSION-009","category":"中间件安全","subsystem":"Apache","name":"会话超时时间设置","severity":"medium","expected":"等于 OK","remediation":"在 httpd.conf 设置 KeepAlive On、KeepAliveTimeout 300、Timeout 300 并重启","reference":"等保2.0 8.1.4"'
_run_item

# ---- APACHE-CONN-010 (中间件安全/Apache) ----
ITEM_CHECK='auto'
ITEM_MTYPE='shell'
ITEM_TIMEOUT='10'
ITEM_FORMAT=''
ITEM_JTYPE='regex_present'
ITEM_TE='fail'
ITEM_CMD='grep -iE "MaxClients|MaxRequestWorkers" $HTTPD_HOME/conf/httpd.conf /etc/httpd/conf/httpd.conf /etc/apache2/apache2.conf 2>/dev/null | grep -vE '\''^#'\'' | head -1'
ITEM_PC=''
ITEM_SECTION=''
ITEM_KEY=''
ITEM_PROC=''
ITEM_PORT=''
ITEM_JVAL='(?i)MaxClients|MaxRequestWorkers'
ITEM_MDESC=''
ITEM_STATIC='"item_id":"APACHE-CONN-010","category":"中间件安全","subsystem":"Apache","name":"限制最大连接数(MaxClients)","severity":"low","expected":"匹配 (?i)MaxClients|MaxRequestWorkers","remediation":"在 httpd.conf 按实际需求配置 MaxClients(或 MaxRequestWorkers) 并重启","reference":"等保2.0 8.1.4"'
_run_item
# ---------------------------------------------------------------------------
# 主机元数据 + 汇总 + 输出
# ---------------------------------------------------------------------------
...
CATALOG_ID="mw_apache"
CATALOG_VER="1.0.0"
VERSIONS='{"db2":"1.0.0","db_mariadb":"1.0.0","db_mongodb":"1.0.0","db_mysql_linux":"1.0.0","db_mysql_win":"1.0.0","db_oracle_linux":"1.0.0","db_oracle_win":"1.0.0","db_postgresql":"1.0.0","db_redis":"1.0.0","db_sqlserver_win":"1.0.0","host_linux":"1.0.0","host_windows":"1.0.0","infra_docker":"1.0.0","infra_k8s":"1.0.0","infra_network":"1.0.0","mw_apache":"1.0.0"}'
